...
/// BaseColor를 제외한 나머지 열 개 값이 실제 화면 토큰입니다.
/// </summary>
...
/// <summary>
/// 저장하지 않은 관리자 테마를 미리보기 요청에만 전달하는 값입니다.
/// </summary>
...
/// <summary>
/// 대표 색상으로부터 WCAG 대비를 고려한 밝은 감사장 팔레트를 생성합니다.
/// 기존 사용자 정의 테마의 필수 5색은 그대로 해석하고, 확장 토큰이 없을 때만
/// 이전 렌더링 규칙으로 폴백합니다.
/// </summary>
public static class WeddingThemePaletteGenerator
{
    ...
    public static WeddingThemePalette Generate(string? baseColor)
    {
        ...
        // 밝은 종이 질감을 유지하되 대표 색상의 색조를 아주 옅게 반영합니다.
        var panel = Mix(white, source, .035);
        var background = Mix(white, source, .12);

        // Primary는 텍스트와 아이콘에도 사용되므로 표면 대비 3:1을 확보합니다.
        ...
        // 어느 대표 색상을 골라도 흰색/검정 중 대비가 높은 쪽을 선택하여
        // 일반 크기 버튼 텍스트의 4.5:1 대비를 확보합니다.
    ...
    /// <summary>
    /// 공개 렌더링용 팔레트입니다. 확장 토큰이 없는 기존 JSON은 종전 규칙과 같은 값으로 폴백합니다.
    /// </summary>
    ...
    /// <summary>
    /// 색상 입력기에 표시할 팔레트입니다. 알파 경계선 폴백은 실제 표면색과 합성한 6자리 HEX로 변환합니다.
    /// </summary>
    ...
    public static double ContrastRatio(string foreground, string background) =>
        ContrastRatio(
            Rgb.Parse(NormalizeHexColor(foreground, "#000000")),
