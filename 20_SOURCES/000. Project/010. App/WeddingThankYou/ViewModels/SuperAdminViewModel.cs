...
{
	/// <summary>
	/// \file SuperAdminViewModel.cs
	/// \brief 등록된 모든 ThankYou 계정(테넌트)을 관리하는 통합 어드민 ViewModel.
	/// \details WeddingPlatform.Web의 WeddingSuperAdminViewModel을 거의 그대로 이식.
	/// </summary>
	public sealed class SuperAdminViewModel
	{
		...
		public SuperAdminViewModel(ITenantStore tenants, WeddingOptions opts, IGlobalSettingsStore globalSettings)
		{
			_tenants = tenants;
			_opts = opts;
		...
		public DateTime NewWeddingDate { get; set; } = DateTime.Today;
		...
		public async Task<bool> CreateTenantAsync(CancellationToken ct = default)
		{
			...
			{ StatusMessage = $"'{slug}'은(는) 슬러그로 사용할 수 없습니다."; return false; }
