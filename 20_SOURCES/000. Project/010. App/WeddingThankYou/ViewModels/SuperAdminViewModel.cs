...
/// <summary>
/// \if KO
/// <para>Super Admin View Model 기능과 관련 상태를 캡슐화합니다.</para>
/// \endif
/// \if EN
/// <para>Encapsulates super admin view model functionality and related state.</para>
/// \endif
/// </summary>
public sealed class SuperAdminViewModel
{
    ...
    /// <summary>
    /// \if KO
    /// <para>지정한 설정으로 <see cref="SuperAdminViewModel"/> 클래스의 새 인스턴스를 초기화합니다.</para>
    /// \endif
    /// \if EN
    /// <para>Initializes a new instance of the <see cref="SuperAdminViewModel"/> class with the specified settings.</para>
    /// \endif
    /// </summary>
    /// <param name="tenants">
    /// \if KO
    ...
    private async Task ReloadTenantsAsync(CancellationToken ct)
    {
        ...
        {
            ThankYouDesignCatalog.Normalize(tenant);
        }
    }
    ...
    public async Task<bool> CreateTenantAsync(CancellationToken ct = default)
    {
        ...
        if (slug is "admin" or "superadmin")
        { StatusMessage = "'admin/superadmin'은 슬러그로 사용할 수 없습니다."; return false; }
        ...
        if (user.IsAuthenticated)
        {
            ...
            config.AdminUsers.Add(new ThankYouAdminUser
            {
                UserId = user.Id,
        ...
        StatusMessage = $"✅ '{slug}' 감사장이 생성되었습니다.";
        return true;
    }
