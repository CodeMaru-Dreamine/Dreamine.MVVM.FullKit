...
namespace WeddingThankYou.ViewModels
{
	/// <summary>
	/// \if KO
	/// <para>\file AdminViewModel.cs \brief 어드민 페이지 ViewModel — 로그인/설정저장/업로드/지오코딩/지도링크생성. \details WeddingPlatform.Web의 WeddingAdminViewModel을 거의 그대로 이식.</para>
	/// \endif
	/// \if EN
	...
	public sealed class AdminViewModel
	{
		...
		/// \if KO
		/// <para>user Context에 사용할 <c>ThankYouUserContext</c> 값입니다.</para>
		/// \endif
		/// \if EN
		/// <para>The <c>ThankYouUserContext</c> value used for user context.</para>
		/// \endif
		/// </param>
		...
		/// <returns>
		/// \if KO
		/// <para>Get Admin Users 작업에서 생성한 <c>List&lt;ThankYouAdminUser&gt;</c> 결과입니다.</para>
		/// \endif
		/// \if EN
		/// <para>The <c>List&lt;ThankYouAdminUser&gt;</c> result produced by the get admin users operation.</para>
		/// \endif
		/// </returns>
		...
		/// <param name="user">
		/// \if KO
		/// <para>user에 사용할 <c>ThankYouCurrentUser</c> 값입니다.</para>
		/// \endif
		/// \if EN
		/// <para>The <c>ThankYouCurrentUser</c> value used for user.</para>
		/// \endif
		/// </param>
		/// <returns>
		/// \if KO
		...
		/// <returns>
		/// \if KO
		/// <para>Build Effective Admin Users 작업에서 생성한 <c>IReadOnlyList&lt;ThankYouAdminUser&gt;</c> 결과입니다.</para>
		/// \endif
		/// \if EN
		/// <para>The <c>IReadOnlyList&lt;ThankYouAdminUser&gt;</c> result produced by the build effective admin users operation.</para>
		/// \endif
		/// </returns>
