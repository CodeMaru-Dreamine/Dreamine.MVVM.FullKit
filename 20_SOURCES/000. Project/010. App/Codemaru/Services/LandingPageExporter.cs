...
public sealed class LandingPageExporter
{
    ...
    public string BuildHtml(CardProfile profile, string qrPayload)
    {
        ...
        var logo = string.IsNullOrWhiteSpace(profile.LogoImageDataUrl)
            ? $"<div class=\"logo-dot\" style=\"background:{Html(profile.AccentColor)}\"></div>"
            : $"<img class=\"logo\" src=\"{Html(profile.LogoImageDataUrl)}\" alt=\"{Html(profile.BackBrand)} logo\" />";
        ...
        html.AppendLine("  <style>");
        html.AppendLine($"    :root {{ --bg: {palette.Background}; --panel: {palette.Panel}; --text: {palette.Text}; --muted: {palette.Muted}; --accent: {Html(profile.AccentColor)}; }}");
        html.AppendLine("    * { box-sizing: border-box; }");
        html.AppendLine("    body { margin: 0; min-height: 100vh; font-family: \"Segoe UI\", \"Noto Sans KR\", sans-serif; background: var(--bg); color: var(--text); display: grid; place-items: center; padding: 28px; }");
        html.AppendLine("    main { width: min(920px, 100%); display: grid; grid-template-columns: .9fr 1.1fr; gap: 18px; background: var(--panel); border: 1px solid rgba(120,140,120,.25); border-radius: 18px; padding: 28px; box-shadow: 0 18px 46px rgba(0,0,0,.14); }");
        html.AppendLine("    .profile { display: grid; align-content: start; gap: 10px; }");
        html.AppendLine("    .logo-wrap { width: clamp(144px, 24vw, 192px); aspect-ratio: 8 / 5; display: grid; place-items: center; border: 2px solid var(--accent); border-radius: 20px; padding: 10px; overflow: hidden; }");
        html.AppendLine("    .logo { display: block; width: 100%; height: 100%; object-fit: contain; border-radius: 6px; }");
        html.AppendLine("    .logo-dot { width: 42px; height: 42px; border-radius: 50%; }");
        html.AppendLine("    .eyebrow { margin: 6px 0 0; color: var(--muted); font-size: 12px; font-weight: 800; text-transform: uppercase; }");
        html.AppendLine("    h1 { margin: 0; font-size: clamp(32px, 6vw, 52px); line-height: 1; }");
        html.AppendLine("    p { margin: 0; color: var(--muted); }");
        html.AppendLine("    a { color: var(--accent); overflow-wrap: anywhere; }");
        html.AppendLine("    dl { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 18px 24px; margin: 0; }");
        ...
        html.AppendLine("    @media (max-width: 720px) { main { grid-template-columns: 1fr; padding: 20px; } dl { grid-template-columns: 1fr; } }");
        html.AppendLine("  </style>");
        html.AppendLine("</head>");
        html.AppendLine("<body>");
        html.AppendLine("  <main>");
        html.AppendLine("    <section class=\"profile\">");
        html.AppendLine($"      <div class=\"logo-wrap\">{logo}</div>");
        html.AppendLine($"      <p class=\"eyebrow\">{Html(profile.BackBrand)}</p>");
        html.AppendLine($"      <h1>{Html(profile.Name)}</h1>");
        html.AppendLine($"      <p>{Html(profile.Role)}</p>");
        html.AppendLine($"      <p>{Html(profile.BackTagline)}</p>");
        html.AppendLine($"      <a href=\"{Html(qrPayload)}\">{Html(qrPayload)}</a>");
        ...
        html.AppendLine($"      <div class=\"wide\"><dt>Description</dt><dd>{Html(profile.LandingDescription)}</dd></div>");
        html.AppendLine("    </dl>");
        html.AppendLine("  </main>");
        html.AppendLine("</body>");
        html.AppendLine("</html>");
