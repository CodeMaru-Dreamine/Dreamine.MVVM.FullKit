...
internal sealed class MultiEquipmentHostOptions
{
    ...
    public int ReconnectQueueCapacity { get; init; } = 4096;
    ...
    public void Validate()
    {
        ...
        if (ReconnectQueueCapacity < ReconnectConcurrency)
            throw new ArgumentOutOfRangeException(nameof(ReconnectQueueCapacity),
                "Reconnect queue capacity must be at least the reconnect concurrency.");
    }
}
...
internal sealed class EquipmentConnectionDefinition
{
    ...
    public void Validate()
    {
        ...
        if (MaximumFrameLength < 10 || MaximumFrameLength > int.MaxValue - 4)
